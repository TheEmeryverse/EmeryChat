...
def get_current_system_prompt(user_query="", user_id=None): # Injects the system prompt into model's context
    ...
    if str(ENABLE_FINANCE).lower() == "true":
        finance_instruction = (
            ...
            "\n- For broad finance topics, prefer the high-level dashboard bundles first instead of manually discovering every series one by one. Use `get_us_macro` for broad U.S. economy, inflation, labor, housing, or consumer questions, `get_market_dashboard` for broad rates, yield-curve, credit, stock-market, volatility, or cross-asset questions, and `get_global_macro` for broad cross-country or global macro questions."
