...
BRIDGE_TOOLS_SCHEMA = [
    {"type": "function", "function": {"name": "tool_search", "description": "Find enabled Emery tools when the tool you need is not already visible. Search with the user's goal in plain language (for example, 'show recent stock prices' or 'list scheduled reminders'). Results are brief metadata only; call tool_describe on the selected tool before using a deferred tool.", "parameters": {"type": "object", "properties": {"queries": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": MAX_QUERIES, "description": "One to eight short natural-language descriptions of the capability you need. Use separate queries for distinct possible capabilities."}, "limit": {"type": "integer", "minimum": 1, "maximum": MAX_RESULTS, "description": "Maximum number of matching tools to return; defaults to 8."}, "include_deferred": {"type": "boolean", "description": "Whether to include specialized deferred tools; defaults to true."}}, "required": ["queries"], "additionalProperties": False}}},
    {"type": "function", "function": {"name": "tool_describe", "description": "Get the complete instructions and JSON argument schema for one tool returned by tool_search. Use this before calling a deferred tool so you know its exact inputs, sequencing rules, and limits.", "parameters": {"type": "object", "properties": {"tool_name": {"type": "string", "maxLength": MAX_QUERY_CHARS, "description": "Exact tool name returned by tool_search."}, "include_schema": {"type": "boolean", "description": "Include the JSON schema for the tool's arguments; defaults to true."}}, "required": ["tool_name"], "additionalProperties": False}}},
    {"type": "function", "function": {"name": "tool_call", "description": "Validate and run one deferred tool after tool_describe has supplied its instructions. Pass the exact tool name and a JSON object matching that tool's schema. Do not use this bridge for eager tools that are already directly available.", "parameters": {"type": "object", "properties": {"tool_name": {"type": "string", "maxLength": MAX_QUERY_CHARS, "description": "Exact deferred tool name returned by tool_search or tool_describe."}, "arguments": {"type": "object", "description": "Arguments for the selected tool, copied from its tool_describe schema. Omit this field only when the tool takes no arguments."}}, "required": ["tool_name"], "additionalProperties": False}}},
]
